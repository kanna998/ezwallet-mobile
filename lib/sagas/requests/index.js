...
import personalSignReq from './personalSignRequest'
import infoReq from './infoRequest'
...
import { storeActivity, updateActivity, openActivity, refreshBalance } from 'uPortMobile/lib/actions/uportActions'
...
import { fetchUrlOrFile } from '../../utilities/ipfs'
import { addMultipleVc } from '../vcSaga'
import requestScreenManager from 'uPortMobile/lib/utilities/requestScreenManager'
...
export function* handleURL({ url, postback, popup, clientId, target, legacyPush }) {
  ...
  if (!requestType) {
    ...
    try {
      const json = yield call(fetchUrlOrFile, url)
      if (
        json &&
        json.status === 'success' &&
        json.message &&
        json.message.content &&
        json.message.content.match(JWT_REGEX)
      ) {
...
export function* messageHandler({ message, popup, target, redirectUrl, postback }) {
  ...
  try {
    const { payload } = yield call(verifyToken, message)
    ...
    if (payload.vc) {
      yield spawn(addMultipleVc, payload.vc)
    }
    ...
    if (handler && handler.handle) {
      ...
      if (request) {
        ...
        console.log("messageHandler : Request",request);

        yield put(storeActivity(request))
        if (popup && AppState.currentState === 'active' && requestType !== 'information' && !handler.hideRequestCard) {
...
export function* returnToApp(url, response) {
  ...
  if (Platform.OS === 'android' && url.protocol === 'https:') {
    try {
      return yield call(IntentLauncher.startActivity, {
  ...
  if (handler && handler.authorize) {
    ...
    if (request.infoType === 'seedRecovery') {
      try {
        ...
        Navigation.mergeOptions('MainTabsId', {
          bottomTabs: {
            currentTabIndex: 4,
          },
        })
...
export function* handleSelectActivity({ activityId }) {
  ...
  if (!!request.type) {
    yield call(requestScreenManager, request.type)
  }
